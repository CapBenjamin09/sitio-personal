...
{{--                    <x-label>Al crear un proyecto se mostrará en el apartado de <b>proyectos realizados</b>.</x-label>--}}
                ...
                @if(session('status'))
                    ...
                    @enderror
                    <form class="space-y-4 md:space-y-6" method="post" action="{{ route('portfolio.update', $project) }}" enctype="multipart/form-data">
                        @csrf
                        @method('patch')
                        ...
                        <x-button type="info" class="grid-cols-1">Guardar</x-button>
